    ...
    try {
      if (isSupabaseConfigured) {
        const { error } = await supabase.auth.signInWithOAuth({
          provider: 'apple',
          // FIX: use the same callback route as Google (was a legacy hash route)
    ...
    try {
      if (mode === 'login') {
        ...
        if (result.success && result.needsConfirmation) {
          // FIX: email confirmation is on, so there is no session yet.
    ...
    <p className="text-center text-xs text-[#163300]/50 mt-5">
      ...
      {mode === 'forgot' && (
        <>
          ...
          <button type="button" onClick={() => switchMode('login')} className="font-bold text-[#163300] hover:text-[#9fe870] transition-colors">
            Back to login
          </button>
        </>
      )}
